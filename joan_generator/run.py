...
# Pobieranie tokena i URL do API Home Assistant z env
SUPERVISOR_TOKEN = os.environ.get('SUPERVISOR_TOKEN')
HASSIO_URL = "http://supervisor/core/api"

def get_ha_entities():
    """Pobiera listę encji z Home Assistant."""
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            # Zwracamy posortowaną listę samych entity_id
            entities = [state['entity_id'] for state in data]
            entities.sort()
            return entities
    except Exception as e:
        print(f"Błąd pobierania encji: {e}")
...
def index():
    ...
    if request.method == 'POST':
        ...
        # 2. Layout i Widgety
        # Dane przychodzą jako JSON string w ukrytym polu input, bo JS buduje strukturę
        ...
        if layout_data_str:
            ...
            processed_widgets = [] # Lista do generowania definicji na dole pliku
            
            for row in layout_rows:
                if not row: continue # Pusty wiersz
                
                # Tworzymy string: entity1, entity2
                row_str = ", ".join([w['id'] for w in row])
                generated_yaml += f"  - {row_str}\n"
                
                # Zbieramy widgety do definicji
                processed_widgets.extend(row)
            
            generated_yaml += "\n# --- DEFINICJE WIDŻETÓW ---\n\n"
            
            # Generowanie definicji widgetów
            # Używamy set() dla ID, żeby nie dublować definicji jeśli ktoś dodał ten sam widget 2 razy
            seen_ids = set()
