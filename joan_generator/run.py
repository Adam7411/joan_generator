...
def generate_joan_dash_yaml(rows, title, grid_params, lang_code, custom_defs, entities_map, device_profile='joan_6', auto_nav_bar=False, available_dash_files=None):
    ...
    def build_icon_style_local(custom_px, suffix=""):
        if custom_px and str(custom_px).strip():
            try:
                px = int(custom_px)
    ...
    try:
        ...
        if rows:
            ...
            for w in processed_widgets:
                ...
                _icon = build_icon_style_local(i_size_custom)
                _icon_off = build_icon_style_local(i_size_custom, "; opacity: 0.5;")
                _icon_off3 = build_icon_style_local(i_size_custom, "; opacity: 0.3;")
