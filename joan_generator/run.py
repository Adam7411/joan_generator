...
# Próba odczytu tokenu z konfiguracji (dla trybu manualnego/lokalnego)
try:
...
def get_ha_entities():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            entities = []
            for state in data:
                # Przekazujemy ID, STAN, ATRYBUTY i JEDNOSTKĘ
                # To jest niezbędne, aby JS mógł wyświetlić "prawdziwy" stan
                entities.append({
                    'id': state['entity_id'],
                    'state': state['state'],
