/* ==========================================
   JOAN 6 GENERATOR - LOGIC
   ========================================== */

// Konfiguracja typów widgetów
...
const i18n = {
    pl: {
        btn_add: "+ DODAJ DO WIERSZA", btn_update: "ZAPISZ ZMIANY", btn_cancel: "Anuluj", 
        btn_new_row: "+ Dodaj Nowy Wiersz", btn_generate: "GENERUJ KOD .DASH",
    ...
    en: {
        btn_add: "+ ADD TO ROW", btn_update: "SAVE CHANGES", btn_cancel: "Cancel", 
        btn_new_row: "+ Add New Row", btn_generate: "GENERATE .DASH CODE",
        sec_add: "2. Add Widget", sec_edit: "2. Edit Widget",
        state_on: "ON", state_off: "OFF", state_open: "OPEN", state_closed: "CLOSED",
        state_locked: "LOCKED", state_unlocked: "UNLOCKED", state_home: "HOME", state_not_home: "AWAY"
    }
};
...
window.addEventListener('DOMContentLoaded', () => {
    // Ustawienie IP w linkach
    const host = window.location.hostname;
    const codePath = document.getElementById('client-path-code');
    const linkDash = document.getElementById('client-dashboard-link');
    if (codePath) codePath.innerText = codePath.innerText.replace('IP_HA', host);
    if (linkDash) {
        linkDash.href = linkDash.href.replace('IP_HA', host);
        linkDash.innerText = linkDash.innerText.replace('IP_HA', host);
    }
    ...
    // Uruchom smart raz na starcie (czyści formularz)
...
function setLang(lang) {
    ...
    // Tłumaczenie statycznych elementów HTML
    const dict = {
        pl: {
            status_ok: "✓ Połączono z Home Assistant", status_error: "⚠ Brak połączenia z API. Sprawdź token.",
            sec_settings: "1. Ustawienia", sec_preview: "3. Podgląd Dashboardu (Kliknij, aby edytować)", sec_code: "Twój kod YAML:",
        ...
        en: {
            ...
            sec_settings: "1. Settings", sec_preview: "3. Dashboard Preview (Click to Edit)", sec_code: "Your YAML Code:",
    ...
    applyTranslations(); // Odświeża przyciski dynamiczne
    renderRows(); // Odświeża stany w podglądzie
}
...
function applyTranslations() {
    ...
    if (editingIndices) {
        btnSpan.innerText = t.btn_update;
        header.innerText = t.sec_edit;
    } else {
        btnSpan.innerText = `${t.btn_add} (${currentLang === 'pl' ? 'Wiersz' : 'Row'} ${rowNum})`;
        header.innerText = t.sec_add;
    }
    
    document.getElementById('btn-cancel').querySelector('span').innerText = t.btn_cancel;
    document.getElementById('btn-new-row').querySelector('span').innerText = t.btn_new_row;
    document.querySelector('button[type="submit"] span').innerText = t.btn_generate;
}

function toggleTheme() {
    const toDark = !document.body.classList.contains('dark-mode');
    document.body.classList.toggle('dark-mode', toDark);
    document.getElementById('btn-theme').innerText = toDark ? '☀️' : '🌙';
...
function applySmartForForm(userAction = false) {
    ...
    // Jeśli użytkownik pisze w polu Encja -> Zaktualizuj Typ
    ...
    // Auto Size
    if ((typeSel.value === 'media_player' || typeSel.value === 'climate' || typeSel.value === 'weather' || typeSel.value === 'clock') && !sizeInput.value) {
        sizeInput.value = '(2x2)';
    }

    // Jeśli użytkownik pisze LUB pole nazwy jest puste -> Zaktualizuj Nazwę i Ikonę
    if (userAction || !nameInput.value.trim()) {
        // Nazwa
        const ent = (window.entitiesData || []).find(e => e.id === idRaw);
...
function pickPreviewIcon(widget) {
    const ent = (window.entitiesData || []).find(e => e.id === widget.id);
...
function saveWidget() {
    ...
    if (type === 'navigate') {
        ...
        if (!dashName) { alert(currentLang === 'pl' ? 'Wymagana nazwa' : 'Required'); return; }
        id = 'navigate.' + dashName;
    } else {
        id = document.getElementById('new-entity-id').value.trim();
        if (!id && !['clock','label','reload'].includes(type)) { alert(currentLang === 'pl' ? 'Wymagana encja' : 'Required'); return; }
        if (!id) id = type + '.' + Date.now();
    }

    let name = document.getElementById('new-widget-name').value.trim();
    if (!name && !['clock','reload'].includes(type)) name = formatNameFromId(id);
...
function clearFormInputs() {
    ...
    document.getElementById('new-widget-type').value = 'switch';
    updateIconPreview();
}
...
function deleteRow(rowIndex) {
    if (editingIndices && editingIndices.r === rowIndex) cancelEdit();
...
function importYaml() {
    ...
    try {
        ...
        Object.entries(defs).forEach(([k, v]) => {
            importedDefsMap[k] = v;
            if (v.entity) {
                importedDefsMap[v.entity] = v;
                const short = v.entity.split('.').pop();
                importedDefsMap[short] = v;
            }
        });
        ...
        layoutLines.forEach(l => {
            ...
            items.forEach(item => {
                ...
                let name = def.title ? def.title : formatNameFromId(rawId);
        ...
        applyTranslations();
        toggleImport();
        alert(currentLang === 'pl' ? 'Import zakończony' : 'Import completed');
    } catch (e) {
        alert((currentLang === 'pl' ? 'Błąd importu: ' : 'Import error: ') + e.message);
    }
    }
    ...
    function prepareAndSubmit(e){
      ...
      try{
        ...
        // Call functions provided in run.py through template OR (since we moved logic to JS) we must emulate them or ensure they are defined here
        // Since buildHeader/buildLayout/buildWidgetsSection are NOT in script.js (they were in previous HTML), we must include them here!
        
        // --- YAML GENERATION LOGIC (MUST BE PRESENT) ---
        const header = [
            ...
            "# --------------------------------", "",
            `title: ${title}`,
            "widget_dimensions: [117, 117]", "widget_size: [2, 1]", "widget_margins: [8, 8]", "columns: 6", `rows: ${(rows.length||8)} # auto`,
        ...
        const layout = rows.map(r=>"  - "+r.map(w=>w.id+(w.size? " "+w.size:"")).join(", ")).join("\n");
        ...
        rows.flat().forEach(w => {
            ...
            if(w.type==='navigate'){
                ...
                if(actionable){
                    ...
                    lines.push(`  icon_on: ${ensureMdi(w.icon_on||pair[0])}`);
                    lines.push(`  icon_off: ${ensureMdi(w.icon_off||pair[1])}`);
                    ...
                    if(w.type==='cover'){
                        lines.push(`    "open": "${t.state_open}"`); lines.push(`    "closed": "${t.state_closed}"`);
                    } else if(w.type==='lock'){
                        lines.push(`    "locked": "${t.state_locked}"`); lines.push(`    "unlocked": "${t.state_unlocked}"`);
                    } else {
                        lines.push(`    "on": "${t.state_on}"`); lines.push(`    "off": "${t.state_off}"`);
                    }
