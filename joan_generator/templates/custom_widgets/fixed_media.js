function fixed_media(widget_id, url, skin, parameters) {
    self = this;
    ...
    function OnPlayButtonClick(self) {
        ...
        if (self.entity_state[self.entity].state !== "playing") {
            if (is_supported(self, "PLAY_MEDIA")) {
                // FIX: Use local variable and clone
                args = JSON.parse(JSON.stringify(self.parameters.post_service_play_pause));
            ...
            if (is_supported(self, "PAUSE")) {
                args = JSON.parse(JSON.stringify(self.parameters.post_service_pause));
                self.call_service(self, args)
            }
            else if (is_supported(self, "STOP")) {
                args = JSON.parse(JSON.stringify(self.parameters.post_service_stop));
                self.call_service(self, args)
            }
            else {
                // Try Play/Pause
    ...
    function OnPreviousButtonClick(self) {
        if (is_supported(self, "PREVIOUS_TRACK")) {
            var args = JSON.parse(JSON.stringify(self.parameters.post_service_previous));
            self.call_service(self, args)
        }
        else {
    ...
    function OnNextButtonClick(self) {
        if (is_supported(self, "NEXT_TRACK")) {
            var args = JSON.parse(JSON.stringify(self.parameters.post_service_next));
            self.call_service(self, args)
        }
        else {
            console.log("NEXT_TRACK attribute not supported")
    ...
    function OnRaiseLevelClick(self) {
        ...
        // FIX: Use local variable and clone
        var args = JSON.parse(JSON.stringify(self.parameters.post_service_level));
